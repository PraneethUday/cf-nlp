...
export default function HomePage() {
  ...
  const ratingChart = useMemo(() => {
    ...
    const labels = ratings.entries.map((e: any) =>
      dayjs.unix(e.ratingUpdateTimeSeconds).format("MMM DD")
    );
  ...
  const verdictPie = useMemo(() => {
    ...
    const hueStep =
      360 /
      (verdicts.length -
        Object.keys(semanticColors).filter((k) => verdicts.includes(k)).length +
        1);
  ...
  const diffBar = useMemo(() => {
    ...
    const data = [
      diff.buckets.lt1100,
      diff.buckets["1100_1399"],
      diff.buckets["1400_1699"],
      diff.buckets["1700_1999"],
      diff.buckets.ge2000,
    ];
    const colors = [
      getRatingColor(1000),
        ...
        {!profile ? (
          ...
          <div className="profile-grid">
            ...
            <div className="profile-stat">
              ...
              <div
                style={{
                  color: profile.currentRating
                    ? getRatingColor(profile.currentRating)
                    : "var(--text-secondary)",
                }}
              >
            ...
            <div className="profile-stat">
              ...
              <div
                style={{
                  color: profile.maxRating
                    ? getRatingColor(profile.maxRating)
                    : "var(--text-secondary)",
                }}
              >
                {profile.maxRating ?? "-"}
            ...
            <div className="profile-stat">
              <b>Join Date</b>
              <div>
                📅 {dayjs.unix(profile.registrationTime).format("MMM DD, YYYY")}
              </div>
            </div>
            <div className="profile-stat">
              <b>Contribution</b>
            ...
            <div className="grid grid-3">
              ...
              <div className="stat-card">
                <div
                  ...
                  style={{
                    color:
                      ratings.summary.avgDelta > 0
                        ? "var(--success-color)"
                        : "var(--danger-color)",
                  }}
                >
                  {ratings.summary.avgDelta > 0 ? "+" : ""}
                  {ratings.summary.avgDelta ?? "-"}
                </div>
              ...
              <tbody>
                {ratings.entries.slice(0, 10).map((e: any) => (
                  <tr key={e.contestId}>
                    ...
                    <td>
                      {dayjs
                        .unix(e.ratingUpdateTimeSeconds)
                        .format("MMM DD, YY")}
                    </td>
                    <td>#{e.rank}</td>
                    <td style={{ color: getRatingColor(e.oldRating) }}>
                      {e.oldRating}
                    </td>
                    <td style={{ color: getRatingColor(e.newRating) }}>
                      {e.newRating}
                    </td>
                    <td
                      style={{
                        color:
                          e.newRating - e.oldRating > 0
                            ? "var(--success-color)"
                            : "var(--danger-color)",
                        fontWeight: "bold",
                      }}
          ...
          <div className="grid grid-2">
            <div className="grid grid-3">
              ...
              <div className="stat-card">
                ...
                >
                  {subs.total
                    ? `${Math.round((100 * subs.accepted) / subs.total)}%`
                    : "-"}
                </div>
                      ...
                      const extraColors = Array.from(
                        {
                          length: Math.max(
                            0,
                            ds.data.length - baseColors.length
                          ),
                        },
                        () =>
        ...
        <section className="card">
          ...
          <div className="grid grid-3">
            <div className="stat-card">
              ...
              >
                {Math.round(
                  ((dayjs().unix() - profile.registrationTime) /
                    (365.25 * 24 * 3600)) *
                    10
                ) / 10}
              </div>
            ...
            <div className="stat-card">
              ...
              >
                {ratings.summary.total
                  ? Math.round((subs.accepted / ratings.summary.total) * 10) /
                    10
                  : "-"}
              </div>
            ...
            <div className="stat-card">
              ...
              >
                {profile.maxRating && profile.currentRating
                  ? Math.round(
                      (profile.currentRating / profile.maxRating) * 100
                    )
                  : "-"}
                %
              </div>
            ...
            <div className="stat-card">
              ...
              >
                {ratings.entries.length > 0
                  ? Math.round(
                      ratings.entries.reduce(
                        (sum: number, e: any) =>
                          sum + Math.abs(e.newRating - e.oldRating),
                        0
            ...
            <div className="stat-card">
              ...
              >
                {Math.round(
                  dayjs().diff(dayjs.unix(profile.registrationTime), "day") /
                    (subs.total || 1)
                )}
              </div>
