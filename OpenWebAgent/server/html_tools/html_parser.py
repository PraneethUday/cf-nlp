...
def _safe_decode(raw: bytes) -> str:
    """Decode HTML bytes using <meta charset=...> if present, else utf-8 (ignore errors)."""
    # Try to detect charset from the head quickly
    head = raw[:4096].decode("latin-1", errors="ignore")
    m = re.search(r'<meta[^>]+charset=["\']?([a-zA-Z0-9_\-]+)', head, flags=re.I)
...
class HtmlParser():
    ...
    def parse_args(self, args: dict[str] = {}) -> None:
        ...
        # [Label]
        label_attr = args.get('label_attr', '') or 'temp_clickable_label'
        ...
        # [id]
        id_attr = args.get('id_attr', '') or 'temp_id'
        regen_id = not attr_check(args.get('id_attr', ''))
    ...
    def set_args(self, use_position: bool = False, window_size: tuple = (), rect_dict: dict[str] = {},
                 label_attr: str = '', id_attr: str = '', keep_attrs: list[str] = [],
                 keep_elem: list[str] = [], obs_elem: list[str] = [], parent_chain: bool = False,
                 get_new_label: bool = False, dataset: str = '') -> None:
    ...
    def ctx2tree(ctx: str) -> html.HtmlElement:
        # strip comments, style, script
        ctx = '' if ctx is None else ctx
        ctx = re.sub(r'<!--[\W\w]*?-->', '', ctx, flags=re.S)
        ctx = re.sub(r'<style[\W\w]*?>[\W\w]*?</style>', '', ctx, flags=re.S | re.I)
        ctx = re.sub(r'<script[\W\w]*?>[\W\w]*?</script>', '', ctx, flags=re.S | re.I)
        # collapse excessive whitespace but keep content readable
        ctx = re.sub(r'\s+', ' ', ctx).strip()

        # robust parser (recover=True tolerates broken HTML)
        parser = html.HTMLParser(encoding='utf-8', recover=True)
        # lxml accepts str; encoding is handled by parser
        return html.fromstring(ctx, parser=parser)
    ...
    def parse(self, root: html.HtmlElement, keep: list[str], obs: list[str], parent_chain: bool = False, get_new_label: bool = False) -> dict[str]:
        def get_text(s: str) -> str:
            return '' if s is None else s.strip()[:500]
        ...
        def _dfs(node: html.HtmlElement, keep: list[str] = [], obs: list[str] = [],
            ...
            # element which is kept equivalent to visible
...
# ---------------------------
# Script entry point (fixes EPIPE & noisy stdout)
# ---------------------------
if __name__ == "__main__":
    try:
        raw = sys.stdin.buffer.read()
        if not raw:
            # No input → empty output so Node won't crash
            print("", end="")
            sys.exit(0)

        html_text = _safe_decode(raw)

        # Minimal args – you can pass richer args by switching your Node call to provide JSON, etc.
        parser = HtmlParser(html_text, args={
            # Keep defaults; set explicit attrs so downstream lookups are stable
            "id_attr": "temp_id",
            "label_attr": "temp_clickable_label",
            # Avoid position gating unless you supply rects
            "use_position": False,
        })

        # If you want aggressive pruning, call parser.prune_tree() here
        result = parser.parse_tree()
        out = result.get("html", "")
        # Print ONLY the parsed text to stdout
        sys.stdout.write(out if out is not None else "")
        sys.stdout.flush()
    except Exception as e:
        # Send all diagnostics to stderr (never stdout)
        sys.stderr.write(f"[html_parser] Error: {repr(e)}\n")
        sys.stderr.flush()
        sys.exit(1)
